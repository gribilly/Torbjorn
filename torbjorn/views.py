...
def get_tags():
    ...
    if parser:
        ...
        for tag_name in parser.tag_names:
            tag_count = parser.get_tag_count(tag_name)
            tag_highlights = sorted(
                parser.get_tag_idxs(tag_name))  # It looks like the highlight plugin needs to have the indices sorted, otherwise weird highlighting will ensue.
            total_tags += len(tag_highlights)
            html_tags.append(dict(name=tag_name, count=tag_count, highlights=tag_highlights))
